...
fireauth.iframeclient.IframeWrapper.prototype.getOptions_ = function () {
  var options = /** @type {!gapi.iframes.OptionsBag} */ ({
    ...
    messageHandlersFilter: /** @type {!gapi.iframes.IframesFilter} */ (
      fireauth.util.getObjectRef("gapi.iframes.CROSS_ORIGIN_IFRAMES_FILTER")
    ),
    attributes: {
      style: {
        position: "absolute",
    ...
    return new goog.Promise(function (resolve, reject) {
      ...
      var onOpen = function (iframe) {
        ...
        iframe
          ...
          .then(clearTimerAndResolve, function (error) {
            reject(new Error("Network Error"));
          });
      };
      /** @type {function():!gapi.iframes.Context} */ (
        fireauth.util.getObjectRef("gapi.iframes.getContext")
      )().open(self.getOptions_(), onOpen);
    });
  });
};
 ...
 */
fireauth.iframeclient.IframeWrapper.prototype.registerEvent = function (
  eventName,
  handler
) {
  var self = this;
 ...
 * @param {gapi.iframes.MessageHandler} handler Message handler.
 */
fireauth.iframeclient.IframeWrapper.prototype.unregisterEvent = function (
  eventName,
  handler
) {
  var self = this;
 ...
 */
fireauth.iframeclient.IframeWrapper.NETWORK_TIMEOUT_ = new fireauth.util.Delay(
  30000,
  60000
);
 ...
 *     with units in ms.
 */
fireauth.iframeclient.IframeWrapper.PING_TIMEOUT_ = new fireauth.util.Delay(
  5000,
  15000
);
...
fireauth.iframeclient.IframeWrapper.loadGApiJs_ = function () {
  ...
  fireauth.iframeclient.IframeWrapper.cachedGApiLoader_ = new goog.Promise(
    function (resolve, reject) {
      ...
      if (fireauth.util.getObjectRef("gapi.iframes.Iframe")) {
        ...
        // timeout.
        var cbName =
          "__iframefcb" + Math.floor(Math.random() * 1000000).toString();
