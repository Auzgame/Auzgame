...
goog.scope(function () {
  ...
  goog.labs.net.webChannel.WebChannelBase = function (
    ...
    this.failFast_ = !!goog.getObjectByName(
      "internalChannelParams.failFast",
      opt_options
    );
    ...
    this.baseRetryDelayMs_ =
      goog.getObjectByName(
        "internalChannelParams.baseRetryDelayMs",
        opt_options
      ) || 5 * 1000;
    ...
    this.retryDelaySeedMs_ =
      goog.getObjectByName(
        "internalChannelParams.retryDelaySeedMs",
        opt_options
      ) || 10 * 1000;
  ...
  WebChannelBase.prototype.connectTest_ = function (testPath) {
    ...
    if (this.httpHeadersOverwriteParam_ && this.extraHeaders_) {
      urlPath = httpCors.setHttpHeadersWithOverwriteParam(
        testPath,
        this.httpHeadersOverwriteParam_,
        this.extraHeaders_
      );
    }
  ...
  WebChannelBase.prototype.connectChannel_ = function () {
    ...
    this.forwardChannelUri_ = this.getForwardChannelUri(
   ...
   * @override
   */
  WebChannelBase.prototype.setHttpSessionIdParam = function (
    httpSessionIdParam
  ) {
 ...
 */
  ...
  WebChannelBase.prototype.setFailFast = function (failFast) {
    ...
    if (
      (this.forwardChannelRequestPool_.hasPendingRequest() ||
      ...
      this.channelDebug_.info(function () {
        return (
          "Retry count " +
          self.forwardChannelRetryCount_ +
          " > new maxRetries " +
          self.getForwardChannelMaxRetries() +
          ". Fail immediately!"
        );
      });

      if (
        !this.forwardChannelRequestPool_.forceComplete(
   ...
   */
  WebChannelBase.prototype.setForwardChannelRequestTimeout = function (
    timeoutMs
  ) {
  ...
  WebChannelBase.prototype.maybeRetryForwardChannel_ = function (request) {
    if (
      this.forwardChannelRequestPool_.getRequestCount() >=
    ...
    if (this.forwardChannelTimerId_) {
      ...
      this.outgoingMaps_ = request
        .getPendingMessages()
        .concat(this.outgoingMaps_);
      return true;
    }

    // No retry for open_() and fail-fast
    if (
      this.state_ == WebChannelBase.State.INIT ||
  ...
  WebChannelBase.prototype.open_ = function () {
    ...
    var requestText = this.dequeueOutgoingMaps_(
      request,
      this.fastHandshake_
        ? this.getMaxNumMessagesForFastHandshake_()
        : WebChannelBase.MAX_MAPS_PER_REQUEST_
    );
  ...
  WebChannelBase.prototype.getMaxNumMessagesForFastHandshake_ = function () {
    ...
    for (var i = 0; i < this.outgoingMaps_.length; i++) {
      ...
      if (
        total === WebChannelBase.MAX_CHARS_PER_GET_ ||
  ...
  WebChannelBase.prototype.makeForwardChannelRequest_ = function (
    ...
    request.setTimeout(
      Math.round(this.forwardChannelRequestTimeoutMs_ * 0.5) +
        Math.round(this.forwardChannelRequestTimeoutMs_ * 0.5 * Math.random())
    );
    this.forwardChannelRequestPool_.addRequest(request);
    request.xmlHttpPost(uri, requestText, true);
  };
  ...
  WebChannelBase.prototype.requeuePendingMaps_ = function (retryRequest) {
    this.outgoingMaps_ = retryRequest
      .getPendingMessages()
      .concat(this.outgoingMaps_);
  };
  ...
  WebChannelBase.prototype.onRequestData = function (request, responseText) {
    ...
    if (
      !request.isInitialResponseDecoded() &&
      this.forwardChannelRequestPool_.hasRequest(request) &&
      ...
      if (this.forwardChannelFlushedCallback_) {
        try {
          ...
          this.channelDebug_.dumpException(
            ex,
            "Exception from forwardChannelFlushedCallback_ "
          );
        }
        // reset
        this.forwardChannelFlushedCallback_ = undefined;
  ...
  WebChannelBase.isFatalError_ = function (error, statusCode) {
    return (
      error == ChannelRequest.Error.UNKNOWN_SESSION_ID ||
  ...
  WebChannelBase.prototype.applyControlHeaders_ = function (request) {
    ...
    if (xhr) {
      var clientProtocol = xhr.getStreamingResponseHeader(
    ...
    for (var i = 0; i < respArray.length; i++) {
      ...
      if (this.state_ == WebChannelBase.State.OPENING) {
        if (nextArray[0] == "c") {
          ...
          if (
            goog.isDefAndNotNull(serverKeepaliveMs) &&
            goog.isNumber(serverKeepaliveMs) &&
            serverKeepaliveMs > 0
          ) {
   ...
   * @private
   */
  WebChannelBase.prototype.startBackchannelAfterHandshake_ = function (
    request
  ) {
  ...
  WebChannelBase.prototype.signalError_ = function (error) {
    ...
    if (error == WebChannelBase.Error.REQUEST_FAILED) {
      ...
      netUtils.testNetwork(
        goog.bind(this.testNetworkCallback_, this),
        imageUri
      );
    } else {
  ...
  WebChannelBase.prototype.getBackChannelUri = function (hostPrefix, path) {
    var uri = this.createDataUri(
      this.shouldUseSecondaryDomains() ? hostPrefix : null,
      path
    );
   ...
   */
  WebChannelBase.prototype.setForwardChannelFlushCallback = function (
    callback
  ) {
