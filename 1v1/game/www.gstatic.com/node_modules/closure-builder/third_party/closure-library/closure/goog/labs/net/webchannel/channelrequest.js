...
goog.labs.net.webChannel.ChannelRequest = function (
  ...
  this.pollingTimer_ = new goog.Timer(
    goog.labs.net.webChannel.environment.getPollingInterval()
  );
...
goog.scope(function () {
  ...
  ChannelRequest.prototype.sendXmlHttp_ = function (hostPrefix) {
    ...
    this.xmlHttp_ = this.channel_.createXhrIo(
      useSecondaryDomains ? hostPrefix : null
    );
    ...
    var headers = this.extraHeaders_
      ? goog.object.clone(this.extraHeaders_)
      : {};
    if (this.postData_) {
      if (!this.verb_) {
        this.verb_ = "POST";
    ...
    try {
      if (xmlhttp == this.xmlHttp_) {
        ...
        this.channelDebug_.warning(
          "Called back with an " + "unexpected xmlhttp"
        );
      }
    } catch (ex) {
  ...
  ChannelRequest.prototype.onXmlHttpReadyStateChanged_ = function () {
    ...
    if (!responseText) {
      ...
      this.channelDebug_.debug(function () {
        return (
          "No response text for uri " +
          channelRequest.requestUri_ +
          " status " +
          status
        );
      });
    ...
    if (this.shouldCheckInitialResponse_()) {
      ...
      if (initialResponse) {
        ...
        this.channelDebug_.warning(
          "XMLHTTP Missing X_HTTP_INITIAL_RESPONSE" + " (" + this.rid_ + ")"
        );
        this.cleanup_();
        this.dispatchFailure_();
        return;
  ...
  ChannelRequest.prototype.startWatchDogTimer_ = function (time) {
    ...
    this.watchDogTimerId_ = requestStats.setTimeout(
      goog.bind(this.onWatchDogTimeout_, this),
      time
    );
  };
